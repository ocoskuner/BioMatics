        ...
        while (std::getline(file, line)) {
            ...
            if (line[0] == '>') {
                ...
                if (currentName.empty()) {
                    logWarning("Empty sequence name found at line " + std::to_string(lineCount) + "; using autogenerated name");
        ...
        for (size_t i = 0; i < sequences.size(); ++i) {
            ...
            for (char c : sequences[i]) {
                if (c != '-' && (c < 'A' || c > 'Z') && (c < 'a' || c > 'z')) {
                    logWarning("Non-standard character '" + std::string(1, c) + "' in sequence " + names[i]);
                }
            }
        }
