...
// E0020 hatasını engellemek için eklenen fonksiyon bildirimi
...
// Eğer yapraksa, sekans ismini yaz
...
vector<vector<double>> computeDistanceMatrix(const vector<string>& seqs) {
    ...
    writeCSV(D, "emd_distance_matrix.csv");
    return D;
}
...
std::vector<double> newDist(N + 1, 0.0);
...
int main() {
    ...
    //  Hem diziler hem isimler alınır
    ...
    vector<vector<double>> D = computeDistanceMatrix(seqs);
    ...
    for (const auto& s : seqs)
        ...
        cout << "\n Guide Tree :\n";
        printAsciiTree(tree, names, nodeDistances);  // Mesafeli görsel
        

        Profile initial = alignFromTree(tree, profiles);
    Profile final = iterativeRefinement(seqs, initial, 3, false);  
    ...
    // double cs = computeCSScore(final.aligned); // ← istersen bu da silinebilir
